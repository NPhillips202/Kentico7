...
/*
====================================================================================================
@name			Courtyard Marriott Denver Downtown - custom.js
@version		1.0.0
...
var win = jQuery(window),
	...
	history_section = jQuery('#history'),
	stay_with_us_section = jQuery('#stay-with-us'),
	meetings_section = jQuery('#meetings'),
	manhattan_section = jQuery('#manhattan'),
...
var slickSlider = function() {
	...
	// history section
	history_section.find('.slick').slick({
		//rtl: false,
		//dots: true,
		//infinite: false,
		speed: 300,
	...
	stay_with_us_section.find('.slick.slick-nav').slick({
		//rtl: false,
		//dots: true,
		//infinite: false,
		lazyLoad: 'ondemand',
		...
		asNavFor: '.slick-for',
		focusOnSelect: true,
		appendArrows: stay_with_us_section.find('.slick-wrapper'),
		responsive: [
			{
				breakpoint: 1000,
				settings: {
		...
		onInit: function(slick) {
			// set vars
			var slick_wrapper = stay_with_us_section.find('.slick-wrapper'),
	...
	stay_with_us_section.find('.slick.slick-for').slick({
		//rtl: false,
		//dots: true,
		//infinite: false,
		speed: 300,
	...
	// meetings section
	meetings_section.find('.slick').slick({
		speed: 300,
		slidesToShow: 1,
		slidesToScroll: 1,
		lazyLoad: 'ondemand',
...
var adjustSections = function() {
	...
	sections.each(function(){
		...
		// #history
		if(jQuery(this).is('#history')) {
			var lt = jQuery('#history').find('.top'),
				lt_h = lt.height(),
		...
		// #stay_with_us
		if(jQuery(this).is('#stay-with-us')) {
			var f = jQuery('#stay-with-us'),
				tl = f.find('.top-left'),
				tr = f.find('.top-right'),
		...
		if(jQuery(this).is('#meetings')) {
			...
			//console.log(mj_div_h);
			mj_div_h = mj_div_h; // re-cache this var, needed in order to populate height
			...
			//console.log('.article: '+ma_h);
			//console.log('.slick-wrapper: '+mj_h);
		}
		// #denver
		if(jQuery(this).is('#denver')) {
			var la =  jQuery('#manhattan'),
				t = la.find('.top'),
				t_h = t.height(),
				t_t = t.css('top');
			//if(viewport() <= 768){
...
// checks if some input type is supported by most of the browsers
// ref: http://stackoverflow.com/questions/10193294/how-can-i-tell-if-a-browser-supports-input-type-date
...
jQuery(function() {	
	...
	// initiate datepicker for page content FORMS
	jQuery('#main .datepicker').datepicker({
		//showOn: "both",
    ...
    // resize elements w/in document when calendar onclick event fires
    jQuery('.fc-event, .fc-button').click(function(){
        adjustSections();
    });
