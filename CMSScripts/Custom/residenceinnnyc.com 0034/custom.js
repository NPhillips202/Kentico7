...
/*
@name			Residence Inn NYC-WTC - custom.js
@version		1.0.0
...
var win = jQuery(window),
	...
	travelers_section = jQuery('#travelers'),
...
var bookingWidgetModal = function() {
	...
	/*book_now_link.on({'click': function(e) {
		...
		closeBookingWidgetModal(booking_mask);
	}});*/
	...
	book_now_link.click(function(e){
		if ( viewport() <= 765 ) {
		  var bookingUrl = 'http://www.marriott.com/nycrl';
			jQuery('#book-now-link').attr('href',bookingUrl);
		} else { 
			e.preventDefault();
		};
...
var slickSlider = function() {
	...
	// travelers section
	travelers_section.find('.slick').slick({
		speed: 300,
		slidesToShow: 1,
		slidesToScroll: 1,
		lazyLoad: 'ondemand',
...
var adjustSections = function() {
	...
	sections.each(function(){
		...
		// #travelers
		if(jQuery(this).is('#travelers')) {
			var m =  jQuery('#travelers'),
				lt = m.find('.left-top'),
				ma = m.find('.article'),
