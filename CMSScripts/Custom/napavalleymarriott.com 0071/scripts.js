// scripts.js (Napa Valley Marriott)
/**
...
var win = jQuery(window),
	...
	kentico_form_wrapper = jQuery('#form'),
	
	header = jQuery('#header'),
	header_top = header.find('.top'),
	...
	main_nav = jQuery('#main-nav').find('.navbar'),
	...
	datepicker_container = jQuery('#ui-datepicker-div').closest('div'),

	//email_signup = jQuery('#header #top-nav .email-sign-up');
	email_signup = header_top.find('.email-sign-up'),
	social_nav = footer.find('.social-nav');

console.timeEnd('vars');
// add up time to get load time
	
	var minorAdjustments = function() {
		// get start time
		console.time('minorAdjustments');
		...
		// Sub-pages ONLY
		var slider_wrapper = jQuery('#header-slider-wrapper, #header-slider .slick-slider .headerImageWrapper');
		var slider_img_height = slider_wrapper.find('img').height();
		//slider_wrapper.height(slider_img_height - 200);
		slider_wrapper.height(slider_img_height);
		...
		console.timeEnd('minorAdjustments');
		// add up time to get load time
	},
	
	minorAdditions = function() {
		// get start time
		console.time('minorAdditions');
		...
		console.timeEnd('minorAdditions');
		// add up time to get load time
	},

	stickyMenu = function() {
		// get start time
		console.time('stickyMenu');
	...
	mmHeaderNav = function() {
			...
			"navbars": [
				{
					"position": "bottom",
					"content": [
						email_signup.clone().wrapInner('<div class="email-sign-up" />').html() + social_nav.clone().wrapInner('<div class="social-nav" />').html(),
		...
		datepicker_container.removeClass('mm-page mm-slideout');
		kentico_form_wrapper.addClass('mm-page mm-slideout');
	...
	megaMenuAdjustment = function() {
		// get start time
		console.time('megaMenuAdjustment');
		...
		console.timeEnd('megaMenuAdjustment');
		// add up time to get load time
	},
	...
	slickSliderAdjustments = function() {
		var slickHeight = jQuery('#header-slider .slick-slider .slider-bg').css('height');
		jQuery('.slick-slider .slick-track').height(slickHeight);
	},
...
//--Form Datepicker
jQuery(function() {
    jQuery(".FormPanel .datepickerForm").datepicker({
		buttonImage: "/getmedia/631a8b39-3bfa-4aac-8290-3e19890b5f7f/cal-icon",
		buttonImageOnly: true,
        showOn: 'both',
        minDate: ('+0D')
    });
    jQuery(".FormPanel .datepickerForm").prop("readonly", true);
});
      
      
// Adding target blank attribute to all a tag on the nappa vallery page 
jQuery(function(){
jQuery(".calender-event-section, .napa-attractions, .nearby-wineries, .nearby-restaurants, .night-life").find("a").attr("target","_blank");
});      