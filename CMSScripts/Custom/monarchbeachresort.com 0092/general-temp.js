jQuery(document).ready(function () {
	...
	/*jQuery("#subscribe_forms").validate({
		ignore: [],
		rules: {
			email: {
				required: true,
		...
		submitHandler: function (form) { // for demo
			jQuery('#subscribe_forms .successmsg').fadeIn();
			setTimeout(function () {
				jQuery('#subscribe_forms .successmsg').fadeOut();

				jQuery('#subscribe_forms')[0].reset();
				jQuery(".valid").removeClass("valid");

				jQuery('#subscribe_forms input').focus().blur();
...
function fixheader() {
	...
	if (scrollTop >= (secondMenu - jQuery("#stickyribbon").outerHeight())) {
		...
		jQuery(".second-menu").css("top", jQuery("#stickyribbon").outerHeight(true));
	} else {
...
var banner_height = 493;//jQuery(".banner-and-guestroom-section").height();
 ...
 function FixedHeader(){
   ...
   //jQuery("header").css({"height",600);}
    jQuery("header").addClass("resizeHeader");
    //jQuery(".logo").addClass("showOther");
   ...
   jQuery(".scrollImage").css("display","block");
 	...
 	jQuery(".header-navigation").addClass("adjust");
        ...
        //jQuery(".logo").removeClass("logoAdjust");
