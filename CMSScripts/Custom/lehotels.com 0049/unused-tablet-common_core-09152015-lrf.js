...
function initMap() {
	var currentProperty = propertyId == 0 ? '' : '?id=' + propertyId;
	$.getJSON(HOST + 'json/properties.json' + currentProperty, function (properties) {
		var image = templateURL + 'assets/desktop/images/blue-dot.png';
		var settings = {
			zoom: 15,
			scrollwheel: false,
		...
		function addMarker(location){
			...
			google.maps.event.addListener(marker, "click", function() {
				infowindow.setContent('<h3>' + location.name + '</h3><p>' + location.street_address + ', ' + location.city_name + ', ' + location.state_abbreviation + ' ' + location.zip + '<br> Phone: ' + location.attributes.phone + '<br><a href="http://maps.google.com/maps?f=d&amp;geocode=&amp;daddr=' + location.location.lat + '%2C%20' + location.location.lng + '&amp;z=14" target="_blank">Get directions</a>');
				infowindow.open(map, marker);
			});
			bounds.extend(latlng);
			if($("#map-canvas").data("propertyid") == "corporate"){
				$("#map-property-"+ location.id).click(function(e){
					e.preventDefault();
					$('html,body').animate({scrollTop: $("#map-canvas").offset().top}, 100);
					infowindow.setContent('<h3>' + location.name + '</h3><p>' + location.street_address + ', ' + location.city_name + ', ' + location.state_abbreviation + ' ' + location.zip + '<br> Phone: ' + location.attributes.phone + '<br><a href="http://maps.google.com/maps?f=d&amp;geocode=&amp;daddr=' + location.location.lat + '%2C%20' + location.location.lng + '&amp;z=14" target="_blank">Get directions</a>');
					infowindow.open(map, marker);
					return false;
				});
...
function run_omniture(type, code) {
...
function loadContent(path, pushToHistory) {
