...
jQuery(function(){
	...
	// ref: https://www.google.com/search?q=add+video+in+slick+slider
	// https://codepen.io/michaelsmyth94/full/EVQEQV/
	// https://stackoverflow.com/questions/31521763/slick-js-and-html5-video-autoplay-and-pause-on-video
	// https://jsfiddle.net/asafusan/38oj9xrd/1/
	// video for homepage
	/*bannerMainNormal.on('afterChange', function(event, slick, currentSlide){
		if (currentSlide == 5) {
			bannerMainNormal.slick('slickPause');
			myVideo.play();
		}
	});
	document.getElementById('myVideo').addEventListener('ended',myHandler,false);
	function myHandler(e) {
		bannerMainNormal.slick('slickPlay');
	}*/

	//initMagnificPopup('.bannerText > a[class^="popup-"]', false);
	...
	bannerMainFullWidth.slick({
		...
		if (currentVideoSlideClass == "youtube" || currentVideoSlideClass == "vimeo"){
			...
			setTimeout(function(){
				...
				bannerMainFullWidth.slick('slickGoTo', slideno + 1);
			},14000);
			currentVideoSlide.find('iframe').attr('src', currentVideoSlide.find('iframe').attr('src').replace("autoplay=0", "autoplay=1"));
			///////////////////////////////
		} else {
			bannerMainFullWidth.slick('slickPlay');
		}
	...
	if(bannerMainFullWidth.find('.slick-slide').first().hasClass('video')){
		...
		bannerMainFullWidth.find('.slick-current iframe').attr('src', bannerMainFullWidth.find('.slick-current iframe').attr('src').replace("autoplay=0", "autoplay=1"));
	...
	function printVideoDuration(iframe, type){
		...
		if(type == "youtube"){
			var duration = ( (duration.split(":")[1] * 60) + duration.split(":")[1] ) *1000;
		}
		parseInt(duration);
		console.log(typeof(duration));
