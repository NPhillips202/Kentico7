	...
	function stickyNav() {
		...
		if (!jQuery('#slideshowWrapper').length > 0 && jQuery('#isMobile').is(':hidden')) {
			var nav = jQuery('#navBorder'),
				leftPos = jQuery('#mainNavWrapper').offset().left,
				topPos = 0,
				bottomOfContent = jQuery('.contentArea').offset().top + jQuery('.contentArea').outerHeight(),
				bottomOfNav = nav.offset().top + nav.outerHeight(),
				bottomDifference = bottomOfContent - bottomOfNav;
			
			// Set positioning
			nav.css('position','fixed');
			nav.css('left',leftPos+'px');
			...
			if (bottomDifference < -1) {
				...
				var newTopPos = topPos + bottomDifference;
				//console.log('newTopPos - ' + newTopPos);
				//nav.css('top', newTopPos+'px');
				nav.animate({'top':newTopPos+'px'}, 1100);
			} else {
				// Else, stick nav to top
				//nav.css('top', topPos+'px');
				nav.animate({'top':topPos+'px'}, 1100);
			};
		};
	};
	jQuery(function() {
