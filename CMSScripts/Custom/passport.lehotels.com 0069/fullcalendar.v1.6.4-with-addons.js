...
function Calendar(element, options, eventSources) {
	...
	function __renderView(inc) { // assumes elementVisible
		ignoreWindowResize++;
...
function compareSlotSegs(seg1, seg2) {
	console.log(seg1.event.sorter, seg2.event.sorter);
	// OLD
...
defaults.titleFormat.list = "MMM d[ yyyy]{ '&#8212;'[ MMM] d yyyy}";
...
function listView(element, calendar) {
	...
	// imports
	__ListView.call(t, element, calendar, 'list');
	var opt = t.opt;
	...
	function render(date, delta) {
		...
		//var start = cloneDate(date, true);
		var start = addDays(cloneDate(date, true), 1);
		...
		//console.log(start);
		//console.log(cloneDate(date, 1));

		t.start = t.visStart = start;
		//t.start = t.visStart = date;
		//t.start = start;
...
function __ListView(element, calendar, viewName) {
	var t = this;
	...
	function buildTable() {
		//t.page = 1;
		body = true;
		//t.title = weekNumberTitle + "Page 1"
	}
	...
	function renderEvents(events, modifiedEventId) {
		...
		var html = $("<ul class='fc-event-container' />");

		// rearrange events array by date
		var newEvents = [];
		var flag = 0;
		for (evt in events) {
			//console.log('evt: ', evt);
			...
			if(evt == 0) {
				newEvents.push(events[evt]);
				//console.log(evt);
			} else {
				for(newEvt in newEvents){
					var eventDateParse = Date.parse(events[evt]['start']);
					var eventDateParseCurrent = Date.parse(newEvents[newEvt]['start']);
					//console.log('eventDateParseCurrent:',eventDateParseCurrent, 'eventDateParse:',eventDateParse);
					if(eventDateParseCurrent > eventDateParse){
						flag = 1;
						newEvents.splice(newEvt, 0, events[evt]);
		...
		for (evt in newEvents) {
			var d, ds, de, dt, tm, id, title, data_date, bgColor;
			var olddt = dt;
			//var event = events[evt];
			var event = newEvents[evt];
			...
			if (event) {
				d = event.start;
				de = event.end;
				dt = formatDate(d, 'dddd, MMM d, yyyy');
				det = formatDate(de, 'dddd, MMM d, yyyy');
				tm = formatDate(d, 'h(:mm)tt');
				id = event.id;
				title = event.title;
				data_date = formatDate(d, 'yyyy-MM-dd');
				bgColor = event.LegendGroupColorCode;

				//if((date - d) <= 0){
				var currentStartDate = Date.parse(formatDate(date, 'yyyy-MM-dd'));
				var startDate = Date.parse(formatDate(d, 'yyyy-MM-dd'));
				var endDate = Date.parse(formatDate(de, 'yyyy-MM-dd'));

				if( ( ( currentStartDate - startDate ) < 0 ) || ( ( endDate - currentStartDate ) > 0 ) ){
					//console.log('future', date + ' - ' + d);
					//console.log('future', Date.parse(formatDate(date, 'yyyy-MM-dd')) + ' - ' + Date.parse(formatDate(d, 'yyyy-MM-dd'));
				} else {
					//console.log('past', date + ' - ' + d);
					//console.log('past', Date.parse(formatDate(date, 'yyyy-MM-dd')) + ' - ' + Date.parse(formatDate(d, 'yyyy-MM-dd'));
					continue;
				}

				//console.log('dC:',date, '\ndS:',d, '\ndE:',de);

				var dayBlockHtml = $("<li class='fc-day-block' />");

				if (dt !== olddt) {
					//dtelem = $("<!--<hr>--><h3>" + title + "</h3>").appendTo(dayBlockHtml);
					//dtelem = $("<!--<hr>--><h3>" + dt + " &mdash; " + det + "</h3>").appendTo(dayBlockHtml);
					dtelem = $("<!--<hr>--><h3>" + dt + "</h3>").appendTo(dayBlockHtml);
				}
				eventelem =
				$("<div>" +
					"<div class='fc-event-inner'>" +
						"<span class='fc-event-time' style='display:none;'>" + tm + "</span>" +
						"<span class='fc-event-title'>" + title + "</span><!--<hr>-->" +
						"<span class='fc-event-date' style='display:block;font-size:11px;font-style:italic; padding: 1px 4px 2px; margin: 3px -4px -2px; background: #eee; color: #333; border: 1px solid " + bgColor + "; border-width: 0 1px 1px; border-bottom-right-radius: 3px; border-bottom-left-radius: 3px;text-shadow:0px 0px 0px;'>" + dt + " &mdash; " + det + "</span>" +
					"</div>" +
				"</div>")
				/*$("<div>" +
					"<div class='fc-event-inner'>" +
						"<span class='fc-event-color' style='background-color:" + bgColor + ";width:5px;height:30px;display:inline-block;vertical-align: middle; margin-right: 10px;'></span>" +
						"<span class='fc-event-time' style='display:inline-block;'>" + dt + " &mdash; " + det + "</span>" +
						"<span class='fc-event-title' style='display:inline-block;'>, " + tm + "</span><!--<hr>-->" +
					"</div>" +
				"</div>")*/
				.attr('id', id)
				...
				.css('color', '#222')
				.css('text-shadow', '0px 0px 0px')*/

				//.css('margin-left', 0)
				//.css('margin-right', 0)

				.addClass('fc-event fc-event-hori')
				.appendTo(dayBlockHtml);

				if(event.LegendGroupName.toLowerCase().indexOf("sales mission") >= 0 && bgColor == "#38D9FE") {
					eventelem.css('color', '#111');
					eventelem.css('text-shadow', '0px 0px 0px');
				}

				dayBlockHtml.appendTo(html);
				eventElementHandlers(event, eventelem);
...
function ListView(element, calendar, viewName) {
	...
	function buildCellHTML(date) {
		...
		// html += "<div class='fc-day-number'>Date." + date.getDate() + "</div>";
...
//defaults.buttonText.weekList = 'list';
...
function WeekListView(element, calendar) {
	...
	//ListEventRenderer.call(t); // taken from fullcalendar.v1.6.0-with-addons.js
	var opt = t.opt;
	var renderList = t.renderList;
	var skipHiddenDays = t.skipHiddenDays;
	var getCellsPerWeek = t.getCellsPerWeek;
...
function agendaListView(element, calendar) {
	...
	___ListView.call(t, element, calendar );
	var opt = t.opt;
...
function ___ListView(element, calendar) {
	var t = this;
...
defaults.buttonText.agendaOne = 'agenda';
...
function agendaListViewOne(element, calendar) {
	...
	// imports
	ListViewSampleOne.call(t, element, calendar, 'agendaOne');
	var opt = t.opt;
...
function compareDaySegments(a, b) {
	...
	//console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!');
	//console.log(a.event.title, a.event.sorter);
	//console.log('###########################');
	//console.log(b.event.title, b.event.sorter);
	//console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!');
