...
var win = jQuery(window),
	...
	minorAdjustments = function() {
		...
		var addtnl_padding = (vp <= 1021 && vp >= 767) ? header_top.outerHeight() + parseInt(header_top.css('margin-bottom')) : 0;
	...
	initSiteMap = function() {
 	...
 	slickSlider = function() {
		...
		win.resize(function(){
			...
			var gallery = jQuery('.optional-gallery.simple-gallery');
			var vp = viewport();
			if(vp < 479) {
    ...
    var initAccordion = function() {
        ...
        jQuery('.accordion-header').click(function (e) {
            if(jQuery(this).is('.inactive-header')) {
                console.log(jQuery(this).is('.inactive-header'));
...
var initMagnificPopup = function(elem, gallery) {

	//jQuery('.video-popup').magnificPopup({
	jQuery(elem).magnificPopup({
		//disableOn: 700, // DO NOT SHOW ON MOBILE 700px width
		type: 'iframe',
		mainClass: 'fade-in-scale', // 'mfp-fade videoHeaderArea_Player',
		removalDelay: 160,
		preloader: true,
...
jQuery(function() {
    ...
    // init virtual tour call
    initMagnificPopup('.virtual-tour-popup', false);

	// add span tag around + sign
	var plus_text = jQuery('#main-nav .navbar > ul > li').find('> a:contains("+")');
	console.log(plus_text.length);
	plus_text.wrapInTag({
		tag: 'span',
		attr: 'class="plus" style="font-size:20px;"',
		words: ['([+])']
	});
    ...
    /*
    // set class name
    jQuery('html').addClass('no-svg');
    // no-svg or svg
    if (document.implementation.hasFeature("http://www.w3.org/TR/SVG11/feature#Image", "1.1")) {
        jQuery('html').addClass('svg').removeClass('no-svg');
    }
    */
...
// CUSTOM - remove .desktop class from footer nav when Width<=768
win.resize(function(){
    var vp = viewport();
    // add .mobile class to #utility-nav
    //if(viewport() < 675) {
    if(vp <= 768) {
        jQuery('#footer .navbar').removeClass('desktop');
    } else {
        jQuery('#footer .navbar').addClass('desktop');
    }
    //adjustSections
}).resize();
