...
/*
@name			Union Station Hotel Nashville - custom.js
@version		1.0.0
...
var slickSlider = function() {
	...
	var banner_slider_wrapper = jQuery('.banner-slider-wrapper');
	var slick_list = banner_slider_wrapper.find('.slick-list');
	var slick_slide = slick_list.find('.slick-slide');
	var slick_img = slick_slide.find('.img');

	/*
	jQuery('#intro-bg').find('.slick').slick({
		infinite: true,
		slidesToShow: 1,
		slidesToScroll: 1,
		autoplay: true,
	...
	*/
	...
	//console.log(intro_bg_section.length);
	//intro_bg_section.find('.slick').slick({
	jQuery('#intro-bg').find('.slick').slick({
		autoplay:true,
		autoplaySpeed:7000,// 7 secs
		speed: 300,
		slidesToShow: 1,
		slidesToScroll: 1,
		lazyLoad: 'ondemand'
	...
	if(jQuery('#intro-bg').length) {
        ...
        win.on('resize', function(){
            ...
            var ctaHeight = (jQuery('.cta-text').length > 0 && jQuery('.cta-text').is(':visible')) ? jQuery('.cta-text').height() : 0;
            jQuery('#intro-bg').height(winH - headerHeight);
            jQuery('#intro-bg').find('.slick .slick-slide').height(winH - headerHeight);
            jQuery('#intro-bg').css('margin-top', parseInt(headerHeight + ctaHeight) + 'px');
        }).trigger('resize');
	}
	...
	/*
	//console.log('yo!!!!');
	jQuery('#intro-bg').find('.slick').on('edge', function(event, slick, direction){
		console.log('edge was hit')
	});
	jQuery('#intro-bg').find('.slick').on('init', function(event, slick, direction){
		// set vars
		//var slick_wrapper = rooms_suites_section.find('.slick-wrapper'),
		//	slick_slide = slick_wrapper.find('.slick-slide');
		console.log('test in intro-bg - ', slick);
	...
	*/
	...
	if(viewport() <= 765){
		...
		slider.slick({
			...
			//variableWidth: true,
			//adaptiveHeight: true,
			slide: 'div.slick-sintrolider-img',//'img',
			//infinite: true,
			/*responsive: [
				{
				breakpoint: 828,
...
// Submit functions
function submitEmailForm(emailForm, defaultValue) {
