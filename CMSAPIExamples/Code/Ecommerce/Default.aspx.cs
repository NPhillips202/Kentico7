...
public partial class CMSAPIExamples_Code_Ecommerce_Default : CMSAPIExamplePage
{
    ...
    private bool AssignDiscountLevelToCustomer()
    {
        ...
        {
            // Get user-site binding
            UserSiteInfo userSite = UserSiteInfoProvider.GetUserSiteInfo(customer.CustomerUserID, CMSContext.CurrentSiteID);

            if (userSite != null)
            {
                // Assign discount level to registered customer
                userSite.UserDiscountLevelID = discountLevel.DiscountLevelID;
    ...
    private bool RemoveDiscountLevelFromCustomer()
    {
        ...
        if (!DataHelper.DataSourceIsEmpty(customers))
        {
            ...
            if (userSite != null)
            {
                // Remove discount level from registered customer
                userSite.UserDiscountLevelID = 0;
                UserSiteInfoProvider.SetUserSiteInfo(userSite);
