...
public static class CMSFunctions
{
    ...
    public static void InitWebFarm()
    {
        ...
        if (AzureHelper.IsRunningOnAzure)
        {
            ...
            WebSyncHelperClass.ServerName = ValidationHelper.GetCodeName(AzureHelper.CurrentInstanceID + "_" + AzureHelper.DeploymentID);
            serverDisplayName = AzureHelper.CurrentInstanceID + "_" + AzureHelper.DeploymentID;
            serverUrl = AzureHelper.CurrentInternalEndpoint;

            createServer = true;
        }
        // Get dynamic name for server name  if enabled in settings or for Azure websites (installed from WWAG package)
        else if (WebSyncHelperClass.GenerateWebFarmServers || SystemDataHelper.GetBoolean(SettingsHelper.AppSettings["CMSWWAGInstallation"], false))
        {
            WebSyncHelperClass.WebFarmEnabled = true;
            serverUrl = URLHelper.GetIPv4Address(HTTPHelper.ServerHostAddress) + SettingsKeyProvider.ApplicationPath.TrimEnd('/') + WebSyncHelperClass.WebFarmUpdaterPage;            
            createServer = true;
