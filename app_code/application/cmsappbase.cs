...
public class CMSAppBase
{
            ...
            using (var h = CMSRequestEvents.Begin.StartEvent(e))
            {
                if (h.Continue)
                {
                    ...
                    if (ConnectionAvailable)
                    {
                        ...
                        {
                            CMSConnectionScope.EnsureRequestScope(null, false,
                            ...
                            if (currentPage != null)
                            {
                                if (AnalyticsHelper.IsLoggingEnabled(siteName, currentPage.NodeAliasPath))
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        VisitorStatusEnum visitorStatus = AnalyticsHelper.GetContextStatus(CMSContext.CurrentSiteName, ref idleExpired);
                                        ...
                                        AnalyticsHelper.CurrentVisitStatus = visitorStatus;
                                    }
                                }
                ...
                if (h.Continue)
                {
                    ...
                    if (ConnectionAvailable)
                    {
                        ...
                        if (sentFromCache && (URLRewriter.CurrentOutputCache > 0) &&
                            (URLRewriter.CurrentStatus != URLRewritingResultEnum.GetFile))
                        {
                        ...
                        // Log page view
                        if (RequestHelper.LogPageHit &&
                            (!QueryHelper.GetBoolean(URLHelper.SYSTEM_QUERY_PARAMETER, false)) &&
                            (status != URLRewritingResultEnum.RESTService))
                        {
                            string siteName = CMSContext.CurrentSiteName;
                            UserInfo currentUser = CMSContext.CurrentUser;
                            PageInfo currentPage = CMSContext.CurrentPageInfo;
    ...
    public static void CMSPreSendRequestHeaders(Object sender, EventArgs e)
    {
        ...
        if (CMSApplicationModule.IsIntegratedMode && !SecurityHelper.IsXFrameOptionsExcluded(URLHelper.CurrentRelativePath, Convert.ToString(URLHelper.Url)))
        {
    ...
    private static void RewriteUrl(URLRewritingResultEnum status, string relativePath, ExcludedSystemEnum excludedEnum, ViewModeOnDemand viewMode, SiteNameOnDemand siteName)
    {
        ...
        if (status == URLRewritingResultEnum.PageNotFound)
        {
