...
" + ControlsHelper.GetPostBackEventReference(this, "#").Replace("'#'", "type") + @"
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        DataClassInfo resolverClassInfo = DataClassInfoProvider.GetDataClass(ClassID);
        if (resolverClassInfo != null)
        {
            ...
            {
                CMS.DocumentEngine.TreeNode node = CMS.DocumentEngine.TreeNode.New(resolverClassInfo.ClassName);
                ...
                {
                    // Prioritize doc. types fields
                    foreach (string prop in node.Generalized.Properties)
                    {
                        if (node.Generalized.PrioritizedProperties.Contains(prop))
                        {
                            resolver.PrioritizeProperty(prop);
                        }
                        else
                        {
                            resolver.SetNamedSourceData(prop, "", false);
                        }
                    }
                }
