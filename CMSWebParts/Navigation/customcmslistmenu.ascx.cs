...
public partial class CMSWebParts_Navigation_customcmslistmenu : CMSAbstractWebPart
{
    ...
    /// <summary>
    /// Gets or sets the HtmlElement.
    /// </summary>
    public string HtmlElement
    {
        get
        {
            return ValidationHelper.GetString(GetValue("HtmlElement"), "");
        }
        set
        {
    ...
    /// <summary>
    /// Gets or sets the targetWindow.
    /// </summary>
    public string targetWindow
    {
        get
        {
            return ValidationHelper.GetString(GetValue("targetWindow"), "");
        }
        set
        {
            SetValue("targetWindow", value);
        }
    }
    ...
    {
        /// <summary>
        /// OnLoad override (set HtmlElement tag inside of a link for navigation)
        /// </summary>
        this.menuElem.LoadDataAutomaticaly = false;
        string returnHtml = "";
        // no html element entered so just pass it through
        if (string.IsNullOrEmpty(HtmlElement))
        {
            returnHtml = this.menuElem.RenderedHTML;
        }
        else
        {
            // modify the html to include the html element entered.
            ...
            System.Text.RegularExpressions.Match match = regexObj.Match(menuHtml);
            while (match.Success)
            {
                string strMatch = match.Groups["text"].Value;
                string newText = "<" + HtmlElement + ">" + strMatch + "</" + HtmlElement + ">";
    ...
    /// <summary>
    /// OnLoad override (set targetWindow attribute)
    /// </summary>
        this.menuElem.LoadDataAutomaticaly = false;
        string returnHtmlTarget = "";
        if (string.IsNullOrEmpty(targetWindow))
        {
            returnHtmlTarget = this.menuElem.RenderedHTML;
        }
        else
        {
            // modify the html to add target attribute
            string menuHtmlTarget = this.menuElem.RenderedHTML;
            returnHtmlTarget = menuHtmlTarget.Replace("<a", "<a target=\"" + targetWindow + "\"");
            menuHtmlTarget = returnHtmlTarget;
        }

        this.menuElem.RenderedHTML = returnHtmlTarget;
    }
