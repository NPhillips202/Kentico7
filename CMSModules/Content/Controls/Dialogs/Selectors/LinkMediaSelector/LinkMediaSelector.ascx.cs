...
public partial class CMSModules_Content_Controls_Dialogs_Selectors_LinkMediaSelector_LinkMediaSelector : LinkMediaSelector
{
    ...
    private string GetSiteWhere()
    {
        ...
        if (!String.IsNullOrEmpty(siteName))
        {
            ...
            where = SqlHelperClass.AddWhereCondition(where, String.Format("SiteName LIKE N'{0}'", SqlHelperClass.GetSafeQueryString(siteName)));
        }
    ...
    private void EnsureLoadedData()
    {
            ...
            !(isLink && CurrentAction.Contains("edit")))
        {
            ...
            if (!URLHelper.IsPostback() && processLoad)
            {
                ...
                // Handle preselection only for content links (from content tree)
                if ((SourceType == MediaSourceEnum.Content) && (IsCopyMoveLinkDialog || IsLinkOutput))
                {
                    HandleDialogSelect();
                }
            }
