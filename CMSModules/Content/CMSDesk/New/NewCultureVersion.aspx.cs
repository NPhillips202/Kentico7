...
public partial class CMSModules_Content_CMSDesk_New_NewCultureVersion : CMSContentPage
{
        ...
        if (radCopy.Checked)
        {
            ...
            if (sourceNode != null)
            {
                if (chkSaveBeforeEditing.Checked && (node != null))
                {
                    ...
                    if ((workflow != null) && workflow.WorkflowAutoPublishChanges && !workflow.UseCheckInCheckOut(CMSContext.CurrentSiteName))
                    {
    ...
    {
        string url;
        if (currentNode.HasSKU && ModuleEntry.IsModuleLoaded(ModuleEntry.ECOMMERCE))
        {
