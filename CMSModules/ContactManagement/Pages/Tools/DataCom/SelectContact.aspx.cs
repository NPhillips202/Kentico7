...
public partial class CMSModules_ContactManagement_Pages_Tools_DataCom_SelectContact : CMSDataComDialogPage
{
    ...
    protected DataSet ContactGrid_OnDataReload(string completeWhere, string currentOrder, int currentTopN, string columns, int currentOffset, int currentPageSize, ref int totalRecords)
    {
        try
        {
            ICredentialProvider credentialProvider = new UserCredentialProvider(CMSContext.CurrentUser);
    ...
    private void RestoreParameters()
    {
        ...
        ContactInfo contact = ContactInfoProvider.GetContactInfo(QueryHelper.GetInteger("ContactID", 0));

        FilterParameter = new ContactFilter { FirstName = contact.ContactFirstName, LastName = contact.ContactLastName, Email = contact.ContactEmail, CompanyName = contact.ContactCompanyName };
