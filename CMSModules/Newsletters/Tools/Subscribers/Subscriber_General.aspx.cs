...
{
    private string emailBeforeSave;
    ...
    /// <summary>
    /// DataForm's OnBeforeSave event handler.
    /// </summary>
    protected void formCustomFields_OnBeforeSave(object sender, EventArgs e)
    {
        emailBeforeSave = SubscriberInfoProvider.GetSubscriberInfo(subscriberId).SubscriberEmail;
    }
    ...
    protected void formCustomFields_OnAfterSave(object sender, EventArgs e)
    {
        if (!formCustomFields.BasicForm.StopProcessing)
        {
            ...
            // Reset subscriber bounces counter when someone changes his e-mail.
            if (emailBeforeSave != subscriberObj.SubscriberEmail)
            {
                subscriberObj.SubscriberBounces = 0;
            }
