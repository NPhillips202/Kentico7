...
public partial class CMSModules_Ecommerce_Controls_ProductOptions_DonationProperties : CMSUserControl
{
    ...
    public void ReloadData()
    {
        ...
        if ((SKU != null) && (SKU.SKUProductType == SKUProductTypeEnum.Donation))
        {
            ...
            amountPriceSelector.ValidationErrorMessage = "Please enter a valid amount.";
            amountPriceSelector.EmptyErrorMessage = "Please enter a valid amount.";

            rvUnits.ErrorMessage = "Please enter a valid number of units.";
    ...
    public string Validate()
    {
        ...
        if (String.IsNullOrEmpty(ErrorMessage) && (SKU != null))
        {
            ...
            if ((minPrice > 0) && (amountPriceSelector.Price < minPrice) || ((maxPrice > 0) && (amountPriceSelector.Price > maxPrice)))
            {
                ...
                {
                    ErrorMessage = String.Format("Please note that the amount must be between {0} and {1}.", fMinPrice, fMaxPrice);
                }
                else if (minPrice > 0.0)
                {
                    ErrorMessage = String.Format("Please note that minimum amount is {0}.", fMinPrice);
                }
                else if (maxPrice > 0.0)
                {
                    ErrorMessage = String.Format("Please note that maximum amount is {0}.", fMaxPrice);
                }
