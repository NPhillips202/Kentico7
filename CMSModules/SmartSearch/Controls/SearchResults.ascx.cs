    ...
    protected void Search()
    {
        ...
        else
        {
            ...
            { 
                ...
                if (CMSContext.ViewMode == ViewModeEnum.LiveSite)
                {
                    ...
                    if (AnalyticsHelper.JavascriptLoggingEnabled(CMSContext.CurrentSiteName))
                    {
                        ...
                        string script = "WebServiceCall('" + URLHelper.GetAbsoluteUrl("~/CMSPages/WebAnalyticsService.asmx") + 
                                        "','LogSearch', '{\"keyword\":" + 
                                        // Serialize raw search text to escape '<' and similar characters, then escape '\'
                                        new JavaScriptSerializer().Serialize(searchText).Replace(@"\", @"\\") + 
                                        ", \"pageGUID\":\"" + CMSContext.CurrentPageInfo.DocumentGUID + "\"}')";
                ...
                // Compute page number when necessary (filter changed the number of found result)
                if ((pgr.PageSize != 0) && (pgr.GroupSize != 0) && (pgr.CurrentPage > ((numberOfResults - 1) / pgr.PageSize) + 1))
                {
                    pgr.CurrentPage = numberOfResults == 0 ? 1 : ((numberOfResults - 1) / pgr.PageSize) + 1;
                }
