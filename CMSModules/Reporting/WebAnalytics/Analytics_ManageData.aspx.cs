...
public partial class CMSModules_Reporting_WebAnalytics_Analytics_ManageData : CMSToolsModalPage
{
    ...
    public void btnDelete_Click(object sender, EventArgs e)
    {
        ...
        if (statCodeName == "campaigns")
        {
            ...
            {
                where = " ((StatisticsCode='campaign' AND StatisticsObjectName ='" + SqlHelperClass.GetSafeQueryString(campaign, false) + "') OR StatisticsCode LIKE 'campconversion;" + SqlHelperClass.GetSafeQueryString(campaign) + "')";
            }
        }
        ...
        if ((statCodeName == "conversion") || statCodeName.StartsWithCSafe("singleconversion", true))
        {
            ...
            if (!statCodeName.StartsWithCSafe("singleconversion", true))
            {
                ...
                {
                    String saveConv = SqlHelperClass.GetSafeQueryString(conversion, false);
            ...
            else
            {
                ...
                if (arr.Length == 2)
                {
                    String saveConv = SqlHelperClass.GetSafeQueryString(arr[1], false);
                    where = String.Format("((StatisticsObjectName = '{0}') AND {1})", saveConv, defaultWhere);
                }
            }
        }
        ...
        {
            where = "StatisticsCode LIKE '" + SqlHelperClass.GetSafeQueryString(statCodeName) + "'";
        }
