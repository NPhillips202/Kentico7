...
public partial class CMSModules_Membership_Pages_Users_User_List : CMSUsersPage
{
    ...
    protected override void CheckPermissions()
    {
        ...
        if (user != null)
        {
            int requestedSiteId = QueryHelper.GetInteger("siteid", 0);
            string requstedSiteName = SiteInfoProvider.GetSiteName(requestedSiteId);

            // Check "read" permission
            if (!user.IsAuthorizedPerResource("CMS.Users", "Read", requstedSiteName))
            {
